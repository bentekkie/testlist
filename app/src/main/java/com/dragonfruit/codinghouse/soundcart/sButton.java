...
public class sButton extends Button implements MediaPlayer.OnCompletionListener, View.OnLongClickListener {
    ...
    public sButton(Context context, AttributeSet attrs) {
        ...
        for (int i = 0; i < N; ++i)
        {
            ...
            switch (attr)
            {
                case R.styleable.sButton_State:
    ...
    public void setDeleteFlag(boolean dFlag){
        this.dFlag = dFlag;
    }
    ...
    public boolean onLongClick(View v) {
        if(this.state != this.Add && !MyActivity.gActionCall) {
            mActionMode = getParent().getParent().getParent().startActionModeForChild((View) getParent().getParent(), mActionModeCallback);

            MyActivity.gActionCall = true;
            gld = ((GridLayout)getParent()).getBackground();
            ((GridLayout)getParent()).setBackgroundColor(Color.BLUE);
            return true;
        }else if(MyActivity.gActionCall){
            return true;
        }else return false;
    }
    Object mActionMode;
        ...
        public boolean onActionItemClicked(ActionMode mode, MenuItem item) {
            switch (item.getItemId()) {
                case R.id.delete_item:
                    dFlag = true;
                    MyActivity.gActionCall = false;
                    mode.finish();
                    ((GridLayout)getParent()).setBackgroundColor(Color.TRANSPARENT);
